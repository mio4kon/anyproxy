  ...
  return function (req, userRes) {
    ...
    const prepareRequestDetail = () => {
      ...
      return Promise.resolve();
         ...
         *
         */
    const checkRequestMockInfo = () => {
      if (path.includes(mockServiceHost)) {
        isLichHost = true;
        const routeUrl = path.split(mockServiceHost)[1];
        switch (routeUrl) {
          case '/clear': {
          ...
          case '/mock': {
            ...
            const reqMockInfoJ = req.headers['mock-data'];
                        //request to mock
            const reqMockInfo = JSON.parse(reqMockInfoJ);
            requsetMockInfo.mockUrl = reqMockInfo.url;
            requsetMockInfo.mockType = reqMockInfo.type;
            requsetMockInfo.table = 'mock';
            logUtil.printLog(color.yellow(`prepare to mock url : [${requsetMockInfo.mockUrl}], type : [${requsetMockInfo.mockType}]`));
            break;
          }
          default:
            isMockData = false;
    ...
    const mockResponse = function *beforeSendResponse(requestD, responseD) {
      const newResponse = responseD.response;
    ...
    co(fetchReqData)
            ...
            .then(co.wrap(function*(userConfig) {
              ...
              if (isMockData) {
                ...
                if (fullUrl.includes(requsetMockInfo.mockUrl)) {
                  logUtil.printLog(color.yellow(`match url: [${path}], query the database for response`));
                  const rows = yield getMockDataFromSql(requsetMockInfo.table, requsetMockInfo.mockUrl, requsetMockInfo.mockType);
                  if (rows.length <= 0) {
                    logUtil.printLog(color.red(`match url: [${path}], but query the database NO response`));
                    return userConfig;
                  } else if (rows.length > 1) {
                    logUtil.printLog(color.red(`match url: [${path}], but query the database has multiple response`));
                    return userConfig;
                  }
            ...
            .then(co.wrap(function*(userConfig) {
              if (userConfig.response) {
                ...
                return {
                  ...
                  _isNeedToMock: userConfig._isNeedToMock,
                };
              } else {
            ...
            .then(co.wrap(function*(responseData) {
              if (responseData._directlyPassToRespond) {
                ...
                return responseData;
              } else if (responseData._isNeedToMock) {
                logUtil.printLog(color.yellow(`mock response for url: [${path}]`));
                return (yield mockResponse(Object.assign({}, requestDetail), Object.assign({}, responseData)))
              } else {
                return responseData;
              }
            }))
